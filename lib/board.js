...
//    return new Board(this.cols, this.rows, this.get.bind(this)); 
  ...
  floodFill(pos, matchfunc, matches = [], checked = new Set()) {
    ...
    while (stack.length > 0) {
      ...
      if (!this.validPos(current)) continue;
  
      const key = current.key;
      if (checked.has(key)) continue;
      checked.add(key);
  
      if (!matchfunc(current)) continue;
  
      matches.push(current);
  
      stack.push(current.left, current.right, current.top, current.bottom);
    }
  }  
  ...
  getCluster(startPos, members, exclusive) {
    ...
    this.floodFill(startPos, filter, result, checked);
    return result;
  ...
  setString(s) {
    if (!s) return;
    const sr = s.trim().split('\n');
    const maxR = Math.min(sr.length, this.rows);
    for (let r = 0; r < maxR; ++r) {
      // Split the row string by whitespace, to get individual cell values
      const tokens = sr[r].trim().split(/\s+/);
      const maxC = Math.min(tokens.length, this.cols);
      for (let c = 0; c < maxC; ++c) {
        let f = tokens[c];
        if (f === '') f = '0';
        this._a[r][c] = parseInt(f, 10);
      }
    }
  }
