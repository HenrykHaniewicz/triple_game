...
function TripleGame(aConfig = defaultConfig) {
  ...
  return {
     ...
     * @returns {void}
     */
    setScore:        function (score) { state.score = score; },
    ...
    */
     ...
     */
    gameOver: function () {
      const emptyTiles = board.getPosList(0).length;
    
      let hasTreasure = false; // Check to make sure there aren't any unopened trasures
      board.forEach((val) => {
        if (val === TRIPLE_FIG.TREASURE || val === TRIPLE_FIG.BIGTREASURE) {
          hasTreasure = true;
        }
      });
    
      let hasRobot = false; // Check if next figure or any figure in store is a robot
      if (state.next === TRIPLE_FIG.ROBOT) {
        hasRobot = true;
      }
      if (state.store && state.store.includes(TRIPLE_FIG.ROBOT)) {
        hasRobot = true;
      }

      return emptyTiles === 0 && !hasTreasure && !hasRobot;
    },
...
// newGame() start new game
  ...
  function setFigure(x,y,f) {
    ...
    while (growto) {
      ...
      if (
        f === TRIPLE_FIG.FLOATINGCASTLE && l.length >= 4 || // only trigger if exactly 4 FLOATINGCASTLEs
        f !== TRIPLE_FIG.FLOATINGCASTLE && l.length >= 3     // keep default for other types
      ) {
        ...
        if (
          f === TRIPLE_FIG.FLOATINGCASTLE && l.length > 4 ||
          f !== TRIPLE_FIG.FLOATINGCASTLE && l.length > 3
        ) growscore += growscore;
        score += growscore;
